...
type Expr_
  ...
  | VarOperator Name.Name ModuleName.Canonical Name.Name Annotation -- CACHE real name for optimization
  | Chr ES.TString
  | Str ES.TString
  | CInt Int
  | CFloat EF.TFloat
  | CList (TList Expr)
  | Negate Expr
  | Binop Name.Name ModuleName.Canonical Name.Name Annotation Expr Expr -- CACHE real name for optimization
...
type Annotation = Forall FreeVars Type


type alias FreeVars = Map.Map Name.Name ()
...
type FieldType = FieldType Int Type
...
type Alias = Alias (TList Name.Name) Type


type Binop = Binop_ Binop.Associativity Binop.Precedence Name.Name


type Union =
  Union
    {- u_vars -} (TList Name.Name)
    {- u_alts -} (TList Ctor)
...
type Ctor = Ctor Name.Name Index.ZeroBased Int (TList Type) -- CACHE length args
...
bAlias =
  B.bin2 Alias (\(Alias a b) -> B.T2 a b)
    (B.bTList Name.bName)
    bType
...
bUnion =
  B.bin4 Union (\(Union a b c d) -> B.T4 a b c d)
    (B.bTList Name.bName)
    (B.bTList bCtor)
    B.bWord64
    bCtorOpts
...
bCtor : B.Binary Ctor
bCtor =
  B.bin4 Ctor (\(Ctor a b c d) -> B.T4 a b c d)
    Name.bName
    Index.bZeroBased
    B.bWord64
    (B.bTList bType)
...
bAnnotation =
  B.bin2 Forall (\(Forall a b) -> B.T2 a b)
  ...
  B.custom "can't happen"
    (\p0 p1 p2 p3 p4 p5 p6 p7 tipe ->
      case tipe of
        ...
        TRecord a b        -> p2 a b
        TUnit              -> p3
        TTuple a b c       -> p4 a b c
        TAlias a b c d     -> p5 a b c d
        TType home name ts ->
          let potentialWord = MList.length ts + 7 in
...
bFieldType =
  B.bin2 FieldType (\(FieldType a b) -> B.T2 a b)
    B.bWord16
    (B.lazy <| \() -> bType)
