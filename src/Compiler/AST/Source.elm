...
type alias Expr = A.Located Expr_
...
type alias Pattern = A.Located Pattern_
...
type Module =
  Module
    {- name    -} (Maybe (A.Located Name.Name))
    {- exports -} (A.Located Exposing)
    {- imports -} (TList Import)
    {- values  -} (TList (A.Located Value))
    {- unions  -} (TList (A.Located Union))
    {- aliases -} (TList (A.Located Alias))
    {- binops  -} (TList (A.Located Infix))
    {- effects -} Effects
...
type Import =
  Import
    {- import -} (A.Located Name.Name)
    {- alias -} (Maybe Name.Name)
    {- exposing -} Exposing


type Value = Value (A.Located Name.Name) (TList Pattern) Expr (Maybe Type)
type Union = Union (A.Located Name.Name) (TList (A.Located Name.Name)) (TList ( A.Located Name.Name, TList Type ))
type Alias = Alias (A.Located Name.Name) (TList (A.Located Name.Name)) Type
type Infix = Infix Name.Name Binop.Associativity Binop.Precedence Name.Name
type Port = Port (A.Located Name.Name) Type
