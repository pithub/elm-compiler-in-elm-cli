...
render (Source sourceLines) ((A.Region (A.Position startLine _) (A.Position endLine _)) as region) maybeSubRegion =
  ...
  case makeUnderline width endLine smallerRegion of
    Nothing ->
      drawLines True width smallerRegion relevantLines (d"")
...
makeUnderline width realEndLine (A.Region (A.Position start c1) (A.Position end c2)) =
  if start /= end || end < realEndLine then
    ...
    let
      spaces = String.repeat (c1 + width + 1) " "
      zigzag = String.repeat (max 1 (c2 - c1)) "^"
    in
...
renderPair ((Source sourceLines) as source) region1 region2 =
  ...
  if startRow1 == endRow1 && endRow1 == startRow2 && startRow2 == endRow2 then
    let
      lineNumber = String.fromInt startRow1
      spaces1 = String.repeat (startCol1 + String.length lineNumber + 1) " "
      zigzag1 = String.repeat (endCol1 - startCol1) "^"
      spaces2 = String.repeat (startCol2 - endCol1) " "
      zigzag2 = String.repeat (endCol2 - startCol2) "^"
    ...
    OneLine <|
      D.vcat
        [ D.hcat [D.fromChars lineNumber, d"| ", D.fromChars line ]
        , D.hcat [D.fromChars spaces1 , D.redS zigzag1,
                  D.fromChars spaces2 , D.redS zigzag2]
        ]
  ...
  case MList.lookup row sourceLines of
    ...
    Just line ->
      case String.uncons (String.dropLeft (col - 1) line) of
        ...
        Just ( c, cs ) ->
          if Var.isUpper c             then Upper c (SE.takeWhile Var.isInner cs)
          else if Var.isLower c        then detectKeywords c cs
          else if Symbol.isBinopChar c then Operator (String.cons c <| SE.takeWhile Symbol.isBinopChar cs)
          else if c == ')'             then Close "parenthesis" ')'
          else if c == ']'             then Close "square bracket" ']'
          else if c == '}'             then Close "curly brace" '}'
          else                              Other (Just c)
...
detectKeywords c rest =
  let
    cs = SE.takeWhile Var.isInner rest
    name = String.cons c cs
  in
  if Set.member name Var.reservedWords
  then Keyword name
  else Lower c name
