...
module Compiler.Elm.Interface exposing
  ...
  , toPublicUnion
  , toPublicAlias
  , DependencyInterface(..), bDependencyInterface
  , public
  , private
  , privatize
  , extractUnion
...
fromModule home (Can.Module _ exports _ unions aliases binops _) annotations =
  Interface
    {- home -} home
    {- values -} (restrict exports annotations)
    {- unions -} (restrictUnions exports unions)
    {- aliases -} (restrictAliases exports aliases)
    {- binops -} (restrict exports (Map.map (toOp annotations) binops))
...
restrictUnions exports unions =
  case exports of
    ...
    Can.Export explicitExports ->
      let
        onLeft = Map.dropMissing identity
        onRight = Map.mapMissing identity (\_ union -> PrivateUnion union)
        onBoth = Map.zipWithMatched identity <| \_ (A.At _ export) union ->
          case export of
            Can.ExportUnionOpen -> OpenUnion union
            Can.ExportUnionClosed -> ClosedUnion union
            _ -> Debug.todo "impossible exports discovered in restrictUnions"
      in
...
restrictAliases exports aliases =
  case exports of
    ...
    Can.Export explicitExports ->
      let
        onLeft = Map.dropMissing identity
        onRight = Map.mapMissing identity (\_ a -> PrivateAlias a)
        onBoth = Map.zipWithMatched identity (\_ _ a -> PublicAlias a)
      in
...
bInterface =
  B.bin5 Interface (\(Interface a b c d e) -> B.T5 a b c d e)
    Pkg.bName
...
bBinop =
  B.bin4 Binop (\(Binop a b c d) -> B.T4 a b c d)
    Name.bName
    Can.bAnnotation
